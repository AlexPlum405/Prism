...
复测环境：Windows 11 显示设置，分辨率 `2560 x 1440 (推荐)`，安装版 Prism。

复测步骤：

1. 在 Windows `设置 > 系统 > 屏幕 > 缩放` 中从 `100% (推荐)` 切到 `125%`。
2. 关闭并重新打开 Prism，加载 `Prism Markdown 语法指南.md`。
3. 采集 Prism 窗口级截图和可访问树。
4. 再切到 `150%`，关闭并重新打开 Prism，重复采集。
5. 复测结束后把系统缩放恢复为 `100% (推荐)`，并重新打开 Prism。

证据截图：

- `screenshots/18-high-dpi-125.jpg`
- `screenshots/18b-high-dpi-150.jpg`

125% 结果：

- 顶部菜单 `文件 / 编辑 / 插入 / 格式 / 导航 / 视图 / 导出 / 窗口 / 帮助` 均可见且可访问。
- 三模式按钮 `编辑 / 分栏 / 预览` 均可见且可访问。
- 文件树、预览正文、滚动条、状态栏 `997 字 · 1:1`、`Markdown 文稿`、`排版` 均可见。
- 未观察到标题栏、菜单、三模式按钮、文件树、正文或状态栏文字重叠 / 裁切 / 溢出。

150% 结果：

- 同一组顶部菜单、三模式按钮、窗口控制按钮、文件树、正文和状态栏控件均可见且可访问。
- 窗口级截图未观察到文字互相覆盖、按钮被挤出或状态栏被裁切。
- 复测后 Windows 显示缩放已恢复到 `100% (推荐)`。

结论：`WIN-UI-004` 从 Blocked 调整为 Pass。
